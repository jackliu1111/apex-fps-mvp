...
func TestMediaWorkflow(t *testing.T) {
	...
	cmd := exec.Command("ffmpeg", "-v", "error", "-f", "lavfi", "-i", "color=black:s=320x180:r=30:d=2", "-c:v", "mpeg4", source)
...
func TestSeparateExportsAndRollback(t *testing.T) {
	...
	for _, name := range []string{"first", "second"} {
		...
		cmd := exec.Command("ffmpeg", "-v", "error", "-f", "lavfi", "-i", "testsrc2=s=320x180:r=30:d=3", "-c:v", "mpeg4", source)
		if out, err := cmd.CombinedOutput(); err != nil {
			t.Fatalf("%v %s", err, out)
		}
		session, err := svc.Analyze(context.Background(), Request{Source: source, Mode: "damage"})
		...
		for i, file := range result.Files {
			...
			if out, err := exec.Command("ffmpeg", "-v", "error", "-i", file, "-f", "null", "-").CombinedOutput(); err != nil {
