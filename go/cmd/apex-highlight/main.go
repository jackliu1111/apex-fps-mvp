...
Commands:
  tui     [--tools-dir DIR] [--output-dir DIR]
  doctor  [--tools-dir DIR]
  probe   [--tools-dir DIR] VIDEO
...
Run without arguments in a terminal to open the selection UI. No Python runtime is required.`)
		return err
	}
	fs := flag.NewFlagSet(args[0], flag.ContinueOnError)
	fs.SetOutput(out)
	dir := fs.String("tools-dir", "", "directory containing ffmpeg and ffprobe")
	...
	case "doctor":
		...
		return writeJSON(out, map[string]any{"ffmpeg": t.FFmpeg, "ffprobe": t.FFprobe, "python_required": false})
	case "probe":
